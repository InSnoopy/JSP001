 ...
 *
 *	컨테이너 빈 관리 정책
 *	1. 특별한 설정(scope)이 없는 한 등록된 모든 빈은 싱글톤으로 관리.
 *		singleton : 빈 하나당 객체가 하나씩 생성 및 관리. 주입 횟수와 상관없이 하나의 객체가 공유. (미리 만들어놓는다.)
 *		prototype : 주입될때마다 새로운 객체가 생성 및 관리됨. 객체의 생성 시점이 지연됨. (사용하기 전까지는 만들지 않는다.)
 *		request/session(web 용) : requestScope, sessionScope의 생명주기와 동일해짐.
 *	2. 특별한 설정(lazy-init)이 없는 한 컨테이너가 초기화될때 등록된 모든 객체를 일시에 생성함.
 *		lazy-init : 주입되기 전까지 생성하지 않는다. prototype이랑 비슷한 타이밍이지만! 이건 singleton이다.
 *	3. 빈들의 객체 생성 순서(depends-on)를 제어할 수 있는 설정이 있음.
 *	4. 컨테이너는 빈의 생명주기 관리할때 callback 구조를 활용함.
